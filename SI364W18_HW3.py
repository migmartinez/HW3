...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/migmartHW3"
## Provided:
...
def customval(username, display_name):
        message = 'Username cannot start with @.'
        message2 = 'Display name must be at least two words'
        if username.data[0] == '@':
            raise ValidationError(message)
        if display_name.data.split() < 2:
...
class TweetForm(FlaskForm):
    ...
    username = StringField("Enter the username of the Twitter user (no '@'!)", validators=[Required(), Length(min=1, max=64), customval])
    display_name = StringField("Enter the display name of the twitter user (must be at least 2 words)", validators=[Required(), customval])
    submit = SubmitField()
...
def index():
    ...
    if user: 
        print("Username already in database", users.id)
    else:
    ...
    tweet = Tweet.query.filter_by(text=text, user_id = users.id).first()
    ...
    if tweet:
        print("Tweet by user already exists", tweets.id)
        return redirect(url_for('/all_tweets'))
    else:
        newTweet = Tweet(text=text, user_id = users.id)
    ...
    return render_template('index.html',text=text,username=username,display_name=display_name) # TODO 364: Add more arguments to the render_template invocation to send data to index.html
...
def see_all_tweets():
    ...
    for tweet in tweets:
        ...
        tweep = (tweets.text, users.username)
        all_tweets.append(tweep)
...
def get_longest_tweet():
    ...
    longest_tweet = ''
    for t in tweets:
        if len(t.text) > len(longest_tweet):
            longest_tweet = t.text
